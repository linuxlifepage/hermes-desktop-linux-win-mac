...
use crate::models::{ConnectionProfile, RemoteSessionStore, UsageSummary};
use crate::remote_python;
use crate::ssh;
use serde::Serialize;

pub async fn load_usage_inner(
    ...
    let payload = UsageSummaryRequest {
        ...
        hinted_session_table: hinted_store.and_then(|store| store.session_table),
    };
    let script = remote_python::wrap_payload(&payload, USAGE_SUMMARY_BODY)?;
    ssh::execute_json::<UsageSummary>(profile, script).await
}
...
const USAGE_SUMMARY_BODY: &str = r#"
import json
import pathlib
import sqlite3
