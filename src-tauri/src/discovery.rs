...
use crate::models::{ConnectionProfile, RemoteDiscovery};
use crate::remote_python::wrap_payload;
use crate::ssh;
use serde::Serialize;

#[derive(Debug, Serialize)]
struct RemoteDiscoveryRequest {
    #[serde(rename = "hermes_home")]
    hermes_home: String,
    #[serde(rename = "profile_name")]
    profile_name: Option<String>,
    #[serde(rename = "custom_home_mode")]
    custom_home_mode: bool,
}

pub async fn discover_inner(profile: ConnectionProfile) -> Result<RemoteDiscovery> {
    let request = RemoteDiscoveryRequest {
        hermes_home: remote_hermes_home_path(&profile),
        profile_name: profile.hermes_profile.clone(),
        custom_home_mode: profile.custom_hermes_home_path.is_some(),
    };
    let script = wrap_payload(&request, DISCOVERY_SCRIPT)?;
    let mut discovery = ssh::execute_json::<RemoteDiscovery>(profile.clone(), script).await?;

    if discovery.active_profile.name.trim().is_empty() {
        discovery.active_profile.name = resolved_hermes_profile_name(&profile);
    }
...
def discover_session_store(hermes_home):
...
def discover_kanban(default_hermes_home, home):
